...
void print_board(float board[26][26],int nlines, int ncols, int nmines){
    ...
    int i = 0, j = 0, z = 0;
    printf("| |");
    while(i < ncols )//print das coordenadas no topo da tabela
    {
        printf("|%c|", letras);
        letras++;
        i++;
    ...
    printf("\n");
    i=0;
    while( i < nlines )// Loop that prints out each character in the board
    {   
        printf("|%d|", i);
        while( j < ncols)
        {
...
void print_final_board(float board[26][26],int nlines, int ncols)//Prints the minefield, showing where all of the mines are placed
{
    int i = 0, j = 0, z = 0;
    char letras = 'A';
    printf("| |");
